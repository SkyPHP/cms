...
(function($) {
	$.fn.saveForm = function(options) {
		...
		return this.submit(function(e) {
			...
			if (!settings.action) settings.action = (!model_name) ? $form.attr('action') : '/aql/save/' + model_name;
			...
			if ($.isFunction(settings.beforeSend2)) {
				settings.beforeSend2($status, doSave, settings.onAbort);
			} else {
				doSave();
			}
			return false;
