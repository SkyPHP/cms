...
(function($) { 
    var JFMFS = function(element, options) {
        ...
        FB.api('/me/friends?fields=' + settings.friend_fields, function(response) {
            /* if these are test users they won't have names, but we can just fill those in with
            stub data to get the friends to show up */
            $.map(  response.data, 
                    function(user){
                        if (! user.name) { 
                            user.name = "test testuser"; user.last_name = "testuser";  
                        } return user; } );
