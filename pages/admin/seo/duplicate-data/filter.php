...
?>
		<div style="margin-bottom:5px;">
		<input type="radio" value="<?=$rs->Fields($filter)?>" id="<?=$filter?>_<?=$rs->Fields($filter)?>" name="<?=$filter?>" style="margin-left:3px;" class="phrase-filter-radio" /> <label style="cursor:pointer;" for="<?=$filter?>_<?=$rs->Fields($filter)?>"><?=strtolower($rs->Fields($filter))?></label>
       	</div>
<?		
		$rs->MoveNext();
	}
